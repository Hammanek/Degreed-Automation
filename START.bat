...
>nul 2>&1 if exist "%CHROMIUM_DIR%\chrome-win\chrome.exe" set CHROMIUM_EXE=%CHROMIUM_DIR%\chrome-win\chrome.exe
>nul 2>&1 if exist "%CHROMIUM_DIR%\chrome-win32\chrome.exe" set CHROMIUM_EXE=%CHROMIUM_DIR%\chrome-win32\chrome.exe

if defined CHROMIUM_EXE goto :browser_ready

echo Prenosny prohlizec nenalezen. Stahuji Chromium...
>nul 2>&1 if not exist "%CHROMIUM_DIR%" mkdir "%CHROMIUM_DIR%"
...
:browser_ready
>nul 2>&1 if not exist "%~dp0browser_profile" mkdir "%~dp0browser_profile"
...
set FIRST_RUN=0
>nul 2>&1 if not exist "%~dp0browser_profile\Default" set FIRST_RUN=1
...
if %FIRST_RUN%==1 (
    ...
    echo.
    >nul 2>&1 start "" "%CHROMIUM_EXE%" %BROWSER_FLAGS% "https://eu.degreed.com/account/login" "%~dp0index.html"
) else (
    >nul 2>&1 start "" "%CHROMIUM_EXE%" %BROWSER_FLAGS% "%~dp0index.html"
)
exit /b
