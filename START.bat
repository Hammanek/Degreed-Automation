...
if exist "%CHROMIUM_DIR%\chrome-win\chrome.exe" set CHROMIUM_EXE=%CHROMIUM_DIR%\chrome-win\chrome.exe >nul 2>&1
if exist "%CHROMIUM_DIR%\chrome-win32\chrome.exe" set CHROMIUM_EXE=%CHROMIUM_DIR%\chrome-win32\chrome.exe >nul 2>&1

if defined CHROMIUM_EXE goto :browser_ready

echo Prenosny prohlizec nenalezen. Stahuji Chromium...
if not exist "%CHROMIUM_DIR%" mkdir "%CHROMIUM_DIR%" >nul 2>&1
...
:browser_ready
if not exist "%~dp0browser_profile" mkdir "%~dp0browser_profile" >nul 2>&1
...
set FIRST_RUN=0
if not exist "%~dp0browser_profile\Default" set FIRST_RUN=1 >nul 2>&1
